...
export class LocalizationService {
  ...
  async getTranslations(language: string): Promise<any> {
    ...
    try {
      ...
      this.logger.warn(
        `Failed to load translations for ${language}: ${error.message}`,
      );
      ...
      throw new NotFoundException(
        `Translations for language "${language}" not found`,
      );
    }
  }
  ...
  detectLanguage(acceptLanguageHeader?: string): string {
    this.logger.debug(
      `Detecting language from header: ${acceptLanguageHeader}`,
    );
  ...
  async getSupportedLanguages(): Promise<string[]> {
    try {
      ...
      this.logger.error(
        `Error reading translations directory: ${error.message}`,
      );
